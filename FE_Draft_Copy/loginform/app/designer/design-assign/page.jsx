...
export default function DesignAssignPage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401 || res.status === 403) {
          ...
          alert("Error: Please log in again to access the photo gallery.");
          return;
        }
  ...
  const updateDesignStatusApi = async (orderDetailId, newStatusKey) => {
    ...
    if (!newStatusCode && newStatusCode !== 0) {
      ...
      alert(`Error: Status ${newStatusKey} invalid.`);
      return false;
    }
  ...
  const handleUploadDesign = async () => {
    ...
    if (!designFile && !selectedImageUrl) {
      alert("Please select a new file or a file from the image library.");
      return;
    }
    ...
    try {
      ...
      alert("The design file has been successfully uploaded and submitted for review!");

      return true;
    } catch (error) {
      console.error("Error when uploading design file:", error);
      alert(`Error when uploading design file: ${error.message}`);
      return false;
    } finally {
  ...
  const handleSendToSellerCheck = async (orderId) => {
    ...
    if (success) {
      ...
      alert(
        "The design has been submitted for review. The order status will be updated."
      );
      fetchTasks();
    }
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="bg-blue-50 rounded-lg shadow-sm border border-blue-100">
            ...
            {!loading && filteredOrders.length === 0 && (
              <div className="p-4 text-center text-slate-500 bg-white rounded-lg">
                No design assignments were found.
              </div>
            )}
            {!loading && filteredOrders.length > 0 && (
              <Table>
                                                    ...
                                                    <div className="grid grid-cols-4 gap-4 max-h-[50vh] overflow-y-auto">
                                                      ...
                                                      0 ? (
                                                        <p className="col-span-4 text-center text-gray-500">
                                                          You haven't uploaded any photos or you can't upload photos.
                                                        </p>
                                                      ) : (
                                                        uploadedImages.map(
                                                          (img, index) => (
                                                ...
                                                {!designFile &&
                                                  !selectedImageUrl && (
                                                    <p className="text-gray-500">
                                                      No design file has been selected yet.
                                                    </p>
                                                  )}
                                              </div>
