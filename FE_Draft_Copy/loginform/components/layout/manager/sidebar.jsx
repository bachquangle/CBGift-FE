...
export default function ManagerSidebar({ 
    ...
    const menuItems = [
        ...
        { id: "manage-order", label: "Manage Order", path: "/manager/manage-order", icon: ClipboardList },
        { id: "manage-account", label: "Manage Account", path: "/manager/manage-account", icon: Users },
        { id: "manage-relationship", label: "Manage Relationship", path: "/manager/manage-relationship", icon: Handshake },
        { id: "manage-catalog", label: "Manage Product", path: "/manager/manage-catalog", icon: Package },
        { id: "manage-category", label: "Manage Category", path: "/manager/manage-category", icon: Tags },
        { id: "report", label: "Reports", path: "/manager/reports", icon: BarChart3 },
        { id: "operations", label: "Operations & Production Report", path: "/manager/operations", icon: Factory },
    ];
    ...
    return (
        ...
        >
            <div 
                className={`p-6 border-b border-indigo-200 bg-gradient-to-r from-indigo-100 to-blue-50 flex ${isLocalCollapsed ? 'justify-center' : 'justify-between'} items-center`}
            >
                {/* Tiêu đề */}
                {!isLocalCollapsed && <h1 className="text-xl font-bold text-indigo-900 whitespace-nowrap">CNC - Manager</h1>}
                ...
                <Button
                    ...
                    className={`p-1 h-auto w-auto text-indigo-800 hover:bg-indigo-200 transition-colors ${isLocalCollapsed ? 'mx-auto' : ''}`}
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-x-hidden">
                {menuItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            {/* ICON */}
                            <IconComponent className={`h-5 w-5 ${!isLocalCollapsed ? 'mr-3' : ''}`} />
                            
                            {/* LABEL */}
                            {!isLocalCollapsed && <span className="truncate">{item.label}</span>}
                        </Button>
                    )
                })}
            </nav>
