...
import { ChevronLeft, ChevronRight, LayoutDashboard, Package, FileText, DollarSign, LocateFixed } from "lucide-react"; // ✨ Import icons
...
export default function SellerSidebar({ currentPage }) {
  ...
  const menuItems = [
    ...
    { id: "manage-order", label: "Manage Order", path: "/seller/manage-order", icon: Package },
    { id: "product-catalog", label: "Product Catalog", path: "/seller/product-catalog", icon: FileText },
    { id: "manage-invoice", label: "Manage Invoice", path: "/seller/manage-invoice", icon: DollarSign },
    { id: "tracking-order-shipping", label: "Tracking", path: "/seller/tracking-order-shipping", icon: LocateFixed },
  ];
  ...
  return (
    ...
    >
      <div 
        className={`p-6 border-b border-indigo-200 bg-gradient-to-r from-indigo-100 to-blue-50 flex ${isCollapsed ? 'justify-center' : 'justify-between'}`}
      >
        {/* Tiêu đề */}
        {!isCollapsed && <h1 className="text-xl font-bold text-indigo-900 whitespace-nowrap">CNC - Seller</h1>}
